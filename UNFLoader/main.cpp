...
void show_title()
{
    ...
    for (i=0; i<titlesize; i++)
    {
        ...
        pdprint(str, 1+(i)%TOTAL_COLORS, 0);
    }

    // Print other stuff
    pdprint("\n--------------------------------------------\n", CR_NONE, 0);
    pdprint("Cobbled together by Buu342\n", CR_NONE, 0);
    pdprint("Compiled on %s\n\n\0", CR_NONE, 1, __DATE__); //TODO: date is always null?!
}
...
void list_args() //TODO: access violation issue is present in the commented text:
{
    ...
    pdprint("  \t %d - %s\n", CRDEF_PROGRAM, 2, CART_64DRIVE1, "64Drive HW1");
    pdprint("  \t %d - %s\n", CRDEF_PROGRAM, 2, CART_64DRIVE2, "64Drive HW2");
    pdprint("  \t %d - %s\n", CRDEF_PROGRAM, 2, CART_EVERDRIVE3, "EverDrive 3.0");
    pdprint("  \t %d - %s\n", CRDEF_PROGRAM, 2, CART_EVERDRIVE7, "EverDrive X7");
    pdprint("  -c <int>\t\t   Set CIC emulation type (64Drive HW2 only)\n", CRDEF_PROGRAM, 0);
    pdprint("  \t 0 - %s\t 1 - %s\n", CRDEF_PROGRAM, 2, "6101 (NTSC)", "6102 (NTSC)");
    pdprint("  \t 2 - %s\t 3 - %s\n", CRDEF_PROGRAM, 2, "7101 (NTSC)", "7102 (PAL)");
    pdprint("  \t 4 - %s\t\t 5 - %s\n", CRDEF_PROGRAM, 2, "x101 (All)", "x102 (All)");
    pdprint("  \t 6 - %s\t\t 7 - %s\n", CRDEF_PROGRAM, 2, "x106 (All)", "5101 (NTSC)");
...
void show_help()
{
    ...
    pdprint("Welcome to the %s!\n", CRDEF_PROGRAM, 1, PROGRAM_NAME_LONG);
    ...
    // Get the category
    pdprint("\nCategory: ", CRDEF_INPUT, 0);
    category = getchar();
    pdprint("%c\n\n", CRDEF_INPUT, 1, category);
    ...
    switch (category)
    {
        ...
        case '4':
            ...
            pdprint("During debug mode, you are able to type commands, which show up in ", CRDEF_PROGRAM, 0);
            pdprint(                                                                    "green", CRDEF_INPUT, 0);
            pdprint(                                                                          " on\n"
                    ...
                    "implement a command to list implemented commands, which is '", CRDEF_PROGRAM, 0);
            pdprint(                                                            "list", CRDEF_INFO, 0);
            pdprint(                                                                 "' by default.\n\n", CRDEF_PROGRAM, 0);
            pdprint("During execution, the ROM is free to print things to the console where this\n"
                    "program is running. Messages from the console will appear in ", CRDEF_PROGRAM, 0);
            pdprint(                                                              "yellow", CRDEF_PRINT, 0);
            pdprint(                                                                     ".\n\n"
